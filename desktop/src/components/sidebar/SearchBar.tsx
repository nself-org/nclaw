import { useState, useEffect, useRef } from 'react';
import { useTopics, SearchResult } from '../../lib/topic-store';
import { Input } from '@/components/ui/input';

interface SearchBarProps {
  onResult(result: SearchResult | null): void;
}

export function SearchBar({ onResult }: SearchBarProps) {
  const [query, setQuery] = useState('');
  const search = useTopics((s) => s.search);
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (timerRef.current) clearTimeout(timerRef.current);
    if (!query.trim()) {
      onResult(null);
      return;
    }
    timerRef.current = setTimeout(async () => {
      const result = await search(query.trim());
      onResult(result);
    }, 200);
    return () => {
      if (timerRef.current) clearTimeout(timerRef.current);
    };
  }, [query, search, onResult]);

  return (
    <div className="px-3 py-2">
      <div className="relative flex items-center">
        <svg
          className="absolute left-3 h-3.5 w-3.5 flex-shrink-0 text-muted-foreground pointer-events-none"
          viewBox="0 0 20 20"
          fill="currentColor"
          aria-hidden="true"
        >
          <path
            fillRule="evenodd"
            d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z"
            clipRule="evenodd"
          />
        </svg>
        <Input
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search topics..."
          aria-label="Search topics"
          data-testid="search-input"
          className="pl-8 pr-8 h-8 text-sm bg-gray-800 border-gray-700 focus-visible:ring-sky-500"
        />
        {query && (
          <button
            onClick={() => setQuery('')}
            aria-label="Clear search"
            className="absolute right-2 text-muted-foreground hover:text-foreground"
          >
            <svg className="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
            </svg>
          </button>
        )}
      </div>
    </div>
  );
}
