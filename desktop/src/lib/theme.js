...
export function applyTheme(mode, accentHex) {
    const root = document.documentElement;
    let effectiveMode = 'dark';
    if (mode === 'system') {
...
export function loadThemeFromStorage() {
    const mode = localStorage.getItem('nclaw.theme.mode') || 'system';
    const accentHex = localStorage.getItem('nclaw.theme.accent') || '#0ea5e9';
    return { mode, accentHex };
}
export function saveTheme(mode, accentHex) {
