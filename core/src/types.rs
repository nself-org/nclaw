...
///
/// Contains metadata about the exchange (title, timestamps, pin state) but not
/// the messages themselves — those are queried separately via `Message`.
...
/// A topic cluster grouping related conversations and memories.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "ts-export", derive(TS))]
#[cfg_attr(
    feature = "ts-export",
...
// Tests live in `tests/types_test.rs` (uses only public API).
